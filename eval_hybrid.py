"""
Evaluation Script for HybridCLIP Model
Tests both ID (In-Distribution) and OOD (Out-of-Distribution) performance
Combines multi-modal and pure visual approaches
"""
...
def process_args():
    parser = argparse.ArgumentParser(description='Evaluates HybridCLIP for OOD detection',
    ...
    parser.add_argument('--name', default="hybrid_eval", type=str, help="unique ID for run")
    ...
    args.log_directory = f"results/{args.in_dataset}/{args.score}/HybridCLIP_{args.CLIP_ckpt_name}_T_{args.T}_ID_{args.name}"
    os.makedirs(args.log_directory, exist_ok=True)
    return args


def create_fewshot_subset_from_dataset(dataset, shots, seed=0):
    """Create a few-shot subset from dataset with balanced samples per class."""
    np.random.seed(seed)
    ...
    for label, indices in class_to_indices.items():
        ...
        selected = indices[:shots]
        fewshot_indices.extend(selected)
    
    return fewshot_indices


def init_prototypes_from_fewshot(model, fewshot_loader, classnames):
    ...
    model.eval()
    with torch.no_grad():
        for images, labels in tqdm(fewshot_loader, desc="Initializing prototypes"):
            images = images.cuda()
            labels = labels.cuda()
            
            # Get visual features (cls + selected patches mean)
            outputs = model(images)
            features = outputs['final_feats_visual']  # Use visual features for prototypes
    ...
    # Initialize prototypes from features
    model.init_prototypes_from_features(all_features, all_labels)
    print(f"✓ Prototypes initialized from {all_features.shape[0]} samples")
...
def evaluate_id_performance(model, test_loader, classnames):
    ...
    with torch.no_grad():
        for images, labels in tqdm(test_loader, desc="Evaluating ID accuracy"):
            images = images.cuda()
            labels = labels.cuda()
            
            # Get combined logits
            outputs = model(images)
            logits = outputs['logits_combined']
...
def main():
    ...
    print("="*80)
    print("Evaluating HybridCLIP Model")
    print("="*80)
    ...
    print(f"Multimodal weight: {args.multimodal_weight}")
    ...
    # Build HybridCLIP model
    print("\nBuilding HybridCLIP model...")
    cfg = {
        'device': device,
        'model_type': 'hybrid',
    ...
    # Load trainable parameters (selector and fuser)
    if 'state_dict' in checkpoint:
        state_dict = checkpoint['state_dict']
        # Load only selector and fuser weights
        model_state = model.state_dict()
        for key in state_dict.keys():
            if key in model_state and (key.startswith('selector.') or key.startswith('fuser.')):
    ...
    # Create args-like object for get_ood_scores_clip
    class Args:
        def __init__(self, score, T, lambda_local):
            self.score = score
            self.T = T
            self.lambda_local = lambda_local
    
    score_args = Args(args.score, args.T, args.lambda_local)
    in_score = get_ood_scores_clip(score_args, model, test_loader, test_labels)
    ...
    for out_dataset in out_datasets:
        ...
        try:
            ...
            out_score = get_ood_scores_clip(score_args, model, ood_loader, test_labels)
            ...
            print(f"Error evaluating {out_dataset}: {e}")
            continue
    
    # Calculate average metrics
    if auroc_list:
        results_dict['avg_auroc'] = np.mean(auroc_list)
        results_dict['avg_aupr'] = np.mean(aupr_list)
        results_dict['avg_fpr95'] = np.mean(fpr_list)
        
        print("\n" + "="*80)
        print("Summary Results")
        print("="*80)
        print(f"ID Accuracy: {id_accuracy:.2f}%")
        print(f"Average AUROC: {results_dict['avg_auroc']:.2f}%")
        print(f"Average AUPR: {results_dict['avg_aupr']:.2f}%")
        print(f"Average FPR95: {results_dict['avg_fpr95']:.2f}%")
        print("="*80)
    
    # Save results
    results_path = os.path.join(args.log_directory, 'results.json')
    with open(results_path, 'w') as f:
        json.dump(results_dict, f, indent=2)
    print(f"\n✓ Results saved to {results_path}")
    
    # Save as dataframe
    save_as_dataframe(results_dict, args.log_directory, 'results.csv')
