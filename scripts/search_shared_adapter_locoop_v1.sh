...
source "$(dirname "${BASH_SOURCE[0]}")/common_params_locoop.sh"

# Method-specific configuration
METHOD="sct-1-2.0-10"
SELECTOR_TYPE="identity"
FUSER_TYPE="shared_adapter"
TEXT_FEATURE_PATH="/amax/yeliu/checkpoints/sct/ViT-B/16/1shots/seed1/text_features_sct.pt"
                                                                    ...
                                                                    for selector_temperature in "${SELECTOR_TEMPERATURE[@]}"; do
                                                                        for patches_per_slot_attn in "${PATCHES_PER_SLOT_ATTN[@]}"; do
                                                                            ...
                                                                            if python3 "$PROJECT_ROOT/src/train_eval.py" \
                                                                                                    ...
                                                                                                    --text_features_path "$TEXT_FEATURE_PATH" \
                                                                                                    --warmup_epochs "$WARMUP_EPOCHS" \
                                                                                                    --warmup_type "$WARMUP_TYPE" 2>&1 | tee -a "$LOG_DIR/grid_search.log"; then
                                                                                                    echo "  ✓ Success"
                                                                                                else
                                                                                                    echo "  ✗ Failed"
                                                                                                fi
