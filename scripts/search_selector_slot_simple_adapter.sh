#!/bin/bash
# ==============================================================================
# Hyperparameter search for selector_slot_simple_adapter method in GL_MCM_FA
# ==============================================================================
...
COUNTER=0
total_combinations=$(( ${#LEARNING_RATES[@]} * ${#BATCH_SIZES[@]} * ${#SEEDS[@]} * ${#NUM_SELECT_VALUES[@]} * ${#LAMBDA_LLM_NEGATIVES[@]} * ${#LAMBDA_MIXUP[@]} * ${#MARGIN_VALUES[@]} * ${#SELECTOR_TEMPERATURE[@]} * ${#PATCHES_PER_SLOT_ATTN[@]} * ${#SLOT_FFN_RATIO[@]} * ${#ADAPTER_RATIO[@]} * ${#LAMBDA_INTRA_CLASS[@]} * ${#INTRA_CLASS_TEMP[@]} ))
                                        ...
                                        for adapter_ratio in "${ADAPTER_RATIO[@]}"; do
                                            for intra_class_lambda in "${LAMBDA_INTRA_CLASS[@]}"; do
                                                for intra_class_temp in "${INTRA_CLASS_TEMP[@]}"; do
                                                    ((COUNTER++))
                                                    echo -e "\n[$COUNTER/$total_combinations] Running: $METHOD - lr=$lr, bs=$bs, seed=$seed, num_select=$num_select, lambda_llm=$llm_neg_lambda, lambda_mixup=$mixup_lambda, margin=$margin_val, sel_temp=$sel_temp, patches_per_slot=$patches_per_slot, slot_ratio=$slot_ratio, adapter_ratio=$adapter_ratio, lambda_intra_class=$intra_class_lambda, intra_class_temp=$intra_class_temp"
