...
class QueryGuidedAttentionFuser(BaseFuser):
    ...
    def forward(self, selected_feats: torch.Tensor, 
        ...
        # Convert to FP32 for MHA and LayerNorm operations
        query_fp32 = query.float()
        selected_feats_fp32 = selected_feats.float()
        
        attn_out_fp32, _ = self.mha(query_fp32, selected_feats_fp32, selected_feats_fp32)
        final_feats_fp32 = self.norm(query_fp32 + attn_out_fp32).squeeze(1)
        
        # Convert back to original dtype
        final_feats = final_feats_fp32.to(dtype)
...
class VisualClassifier(nn.Module):
    ...
    def forward(self, features: torch.Tensor) -> torch.Tensor:
        ...
        # Ensure prototypes are normalized and stable
        # Add NaN/Inf check and normalization before use
        prototypes = self.prototypes.clone()
        prototypes = torch.nan_to_num(prototypes, nan=0.0, posinf=1.0, neginf=-1.0)
        prototypes_norm = F.normalize(prototypes, dim=-1, eps=1e-8)
...
class ModularCustomCLIP(nn.Module):
    ...
    def forward(self, image: torch.Tensor, labels: Optional[torch.Tensor] = None, 
        ...
        if self.cfg.get('use_semantic_exclusion', False) and labels is not None:
            ...
            aux_losses['semantic_exclusion'] = self.cfg.get('lambda_llm_negatives', 0.1) * sem_loss
