# Shared Adapter + LoCoOp OOD Regularization 使用指南

## 概述

实现了 Shared Adapter 架构，让 CLS token 和 patch token 共用同一个 adapter，确保它们在同一个特征空间中，这样正则化损失可以更好地让它们互相约束。

## 核心改进

### 1. SharedAdapterFuser 类

新增了 `SharedAdapterFuser` 类，CLS token 和 patch token 共用同一个 adapter：

```python
class SharedAdapterFuser(BaseFuser):
    """
    Shared Adapter that processes both CLS token and patch tokens with a same adapter.
...
**特点**：
- **共用 adapter**：CLS 和 patches 使用同一个 `shared_adapter`
- **分别的归一化**：CLS 和 patches 分别使用 `cls_norm` 和 `patch_norm`
- **零初始化**：输出层零初始化，保证初始阶段不破坏原特征

**优势**：
1. **特征空间一致**：CLS 和 patches 在同一个特征空间中
2. **正则化有效**：LoCoOp 损失可以更好地约束 CLS 和 patches
3. **参数效率高**：相比独立 adapter，参数量减半
4. **训练稳定**：共享参数减少了过拟合风险
...
if isinstance(self.fuser, SharedAdapterFuser):
    # 使用 shared adapter，分别计算 CLS 和 patch 损失
...
- `--fuser_type shared_adapter`: 使用 SharedAdapterFuser（推荐）
...
- `--lambda_locoop_cls`: CLS token OOD 损失权重（默认 0.5，仅在 shared_adapter 模式下有效）
- `--lambda_locoop_patch`: Patch token OOD 损失权重（默认 0.5，仅在 shared_adapter 模式下有效）
...
**关键区别**：
- **Shared Adapter**：CLS 和 patches 使用同一个 adapter，参数共享
- **Dual Adapter**（废弃）：CLS 和 patches 使用独立的 adapter，参数不共享
...
1. **CLS Token 处理**：
   - CLS token 通过 `shared_adapter`
   - 计算 CLS token 与所有类别的相似度
   - 识别 OOD samples（top-k 中不包含真实标签）
   - 计算 OOD samples 的熵

2. **Patch Tokens 处理**：
   - Patch tokens 通过**同一个** `shared_adapter`
...
- **关键**：由于共用 adapter，CLS 和 patches 互相约束，建议 `lambda_cls` 和 `lambda_patch` 保持平衡
...
2. **Shared Adapter only**：
   ```bash
   --fuser_type shared_adapter --selector_type identity
   ```

3. **Shared Adapter + LoCoOp**：
   ```bash
   --fuser_type shared_adapter --selector_type identity --use_locoop_ood
   ```
...
1. **ID 分类准确率**：
   - Shared Adapter 可能略微提升（因为参数共享，泛化能力更强）
   - LoCoOp 正则化可能略微降低（因为增加了约束）

2. **OOD 检测性能**：
   - CLS OOD 损失：提升全局特征的 OOD 检测能力
   - Patch OOD 损失：提升局部特征的 OOD 检测能力
   - **共享特征空间**：CLS 和 patches 互相约束，OOD 检测更鲁棒
   - 组合效果：预期 AUROC 和 FPR95 指标提升

### 计算开销

- **Shared Adapter**：比 Simple Adapter 略慢（需要处理 CLS 和 patches），但比 Dual Adapter 快（参数少）
- **LoCoOp 损失**：需要计算所有 patches 的相似度，可能增加训练时间

## 注意事项

1. **内存使用**：
   - Shared Adapter 需要存储 CLS 和 patches 的适配后特征
...
3. **超参数敏感性**：
   ...
   - 建议进行网格搜索找到最佳配置
   - **共享 adapter**：建议 `lambda_cls` 和 `lambda_patch` 保持平衡
...
Shared Adapter + LoCoOp OOD Regularization 提供了一个高效且鲁棒的框架，可以：

1. **参数共享**：CLS 和 patches 共用同一个 adapter，参数量减半
2. **特征空间一致**：CLS 和 patches 在同一个特征空间中
3. **互相约束**：LoCoOp 损失可以更好地约束 CLS 和 patches
4. **灵活配置**：通过超参数控制 CLS 和 patches 损失的权重
5. **与现有损失函数无缝集成**

这个架构特别适合你的需求：不使用 selector，让 CLS 和 patches 都通过同一个 adapter 进行处理，并分别计算对应的损失，同时确保它们在同一个特征空间中互相约束。
